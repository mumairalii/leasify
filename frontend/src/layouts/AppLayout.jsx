...
function AppLayout() {
  ...
  // REVERTED: Simple NavItem without integrated tooltip
  ...
  // Tooltip wrapped NavItem for the collapsed sidebar
  const CollapsedNavItem = ({ to, icon, children }) => (
    <Tooltip>
      <TooltipTrigger asChild>
        <NavItem to={to} icon={icon} isCollapsed={true} />
      </TooltipTrigger>
      <TooltipContent side="right">{children}</TooltipContent>
    </Tooltip>
  );

  // NavHeading component for section titles in the sidebar
  ...
  // Main navigation for expanded and mobile views
  const NavContent = () => {
      ...
      return (
        <nav className="flex flex-col text-sm font-medium">
          ...
          <div className="px-2 space-y-1">
            ...
            <NavItem to="/landlord/properties" icon={Building2}>
              Properties
            </NavItem>
            <NavItem to="/landlord/tenants" icon={Users}>
              Tenants
            </NavItem>
            {/* --- ADDED PAYMENTS LINK HERE --- */}
            <NavItem to="/landlord/payments" icon={CircleDollarSign}>
              Payments
            </NavItem>
            <NavItem to="/landlord/applications" icon={FileText}>
              Applications
            </NavItem>
    ...
    if (user?.role === "tenant") {
      return (
        <nav className="flex flex-col text-sm font-medium">
          <NavHeading>My Home</NavHeading>
          <div className="px-2 space-y-1">
            <NavItem to="/tenant/dashboard" icon={Home}>
              Dashboard
            </NavItem>
            <NavItem to="/tenant/lease" icon={FileText}>
              My Lease & Payments
            </NavItem>
            <NavItem to="/tenant/payment-history" icon={FileText}>
              Payment History
            </NavItem>
            <NavItem to="/tenant/maintenance/new" icon={Wrench}>
              Maintenance
            </NavItem>
          </div>
  ...
  // Separate navigation for the collapsed sidebar view
  const CollapsedNavContent = () => {
    if (user?.role === "landlord") {
      return (
        <nav className="grid gap-1 px-2 text-sm font-medium">
          <CollapsedNavItem to="/landlord/dashboard" icon={Home}>
            Dashboard
          </CollapsedNavItem>
          <CollapsedNavItem to="/landlord/properties" icon={Building2}>
            Properties
          </CollapsedNavItem>
          <CollapsedNavItem to="/landlord/tenants" icon={Users}>
            Tenants
          </CollapsedNavItem>
          {/* --- AND ADDED PAYMENTS LINK HERE --- */}
          <CollapsedNavItem to="/landlord/payments" icon={CircleDollarSign}>
            Payments
          </CollapsedNavItem>
          <CollapsedNavItem to="/landlord/applications" icon={FileText}>
            Applications
          </CollapsedNavItem>
          <CollapsedNavItem to="/landlord/maintenance" icon={Wrench}>
            Maintenance
          </CollapsedNavItem>
        </nav>
      );
    ...
    if (user?.role === "tenant") {
      return (
        <nav className="grid gap-1 px-2 text-sm font-medium">
          <CollapsedNavItem to="/tenant/dashboard" icon={Home}>
            Dashboard
          </CollapsedNavItem>
          <CollapsedNavItem to="/tenant/lease" icon={FileText}>
            My Lease & Payments
          </CollapsedNavItem>
          <CollapsedNavItem to="/tenant/payment-history" icon={FileText}>
            Payment History
          </CollapsedNavItem>
          <CollapsedNavItem to="/tenant/maintenance/new" icon={Wrench}>
            Maintenance
          </CollapsedNavItem>
        </nav>
      );
  ...
  return (
    <div className="grid min-h-screen w-full lg:grid-cols-[auto_1fr]">
      ...
      <aside className="hidden border-r bg-card text-card-foreground lg:block">
        <div
          className={cn(
            "flex h-full max-h-screen flex-col gap-2 p-4 transition-all duration-300 ease-in-out",
...
//   // Main navigation content, now with a dedicated and structured view for tenants
//   const NavContent = () => {
//     if (user?.role === "landlord") {
//       return (
//         <nav className="flex flex-col text-sm font-medium">
...
//       );
//     }
//     return null; // Render nothing if role is not determined
//   };

//   // Flat navigation links for the collapsed sidebar view
//   const renderFlatNavLinks = () => {
//     if (user?.role === "landlord") {
//       return (
//         <>
//           <NavItem to="/landlord/dashboard" icon={Home}></NavItem>
//           <NavItem to="/landlord/properties" icon={Building2}></NavItem>
//           <NavItem to="/landlord/tenants" icon={Users}></NavItem>
//           <NavItem to="/landlord/applications" icon={FileText}></NavItem>
...
//       return (
//         <>
//           <NavItem to="/tenant/dashboard" icon={Home}></NavItem>
//           <NavItem to="/tenant/lease" icon={FileText}></NavItem>
//           <NavItem to="/tenant/maintenance/new" icon={Wrench}></NavItem>
//         </>
//       );
//     }
//     return null;
...
//         <div
//           className={cn(
//             "flex h-full max-h-screen flex-col gap-2 p-4",
//             sidebarCollapsed ? "w-[72px]" : "w-[280px]"
//           )}
//         >
//           <div className="flex h-14 items-center border-b px-2 justify-between mb-2">
